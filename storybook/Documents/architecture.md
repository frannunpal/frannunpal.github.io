...
Electron builds produce unsigned `.dmg` installers for both architectures:

- `arm64` (Apple Silicon) and `x64` (Intel)
- Built on `macos-latest` runner in CI/CD
- First launch requires right-click > Open due to missing code signing
...
- **release.yml**: npm publish step moved after create-tag to prevent electron build failures from blocking npm publication
...
Electron builds produce `.dmg` installers for macOS:

- **Two architectures**: `arm64` (Apple Silicon) and `x64` (Intel)
- **CI/CD**: Builds run on `macos-latest` runner
- **Unsigned**: Built with `identity: null` — users must right-click > Open on first launch
- **Artifact naming**: `${productName}-${version}-mac-${arch}.dmg`
