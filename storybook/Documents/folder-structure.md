...
│   │   ├── useClock.ts
│   │   ├── useDownload.ts
│   │   ├── useFileOperations.ts    # File move/copy/replace for drag & drop
│   │   ├── useGlobalDropHandler.ts  # Central handler for file drop events
│   │   ├── useDropMenu.tsx          # Drop context menu state management
│   │   ├── usePendingFsDropConsumer.ts # Consumes pending drops in Files app
│   │   ├── useDragDrop/             # Drag & drop subsystem
│   │   │   ├── DragDropStateContext.tsx
│   │   │   ├── useWindowDropZone.ts  # Makes windows drop targets
│   │   │   ├── buildDragItem.ts      # Factory for DragItemData
│   │   │   ├── constants.ts
│   │   │   ├── types.ts
│   │   │   └── index.ts
│   │   └── ...
...
│   │   ├── uuid.ts
│   │   ├── formatBytes.ts
│   │   ├── gradient.ts         # Gradient CSS builders and parsers
│   │   ├── createNoise.ts     # Noise texture generator for wallpapers
│   │   ├── resolveFileContent.ts # Resolve file content for drag & drop
│   │   ├── buildBreadcrumbs.ts # Build path/breadcrumbs from file nodes
│   │   └── ...
│   │
...
| **Hooks**          | `useOpenApp`, `useNotifications`, `useSystemTheme`, `useClock`, `useDownload`, `useIsDirty`, `useFileOperations`, `useGlobalDropHandler`, `useDropMenu`, `useWindowDropZone`, etc. |
...
│   │   └── sections/WPModule/ # Wallpaper modules (Static, Gradient)
...
│   ├── WindowDropOverlay.tsx # Drop target overlay for windows
│   └── ...
├── FdeDesktop/         # Main wrapper component
├── Desktop/            # Desktop area
...
└── Shared/             # Reusable UI components
    ├── DropFileMenu/       # Context menu for file drag & drop
    ├── UnsavedChangesModal/ # Unsaved changes confirmation dialog
    ├── GradientPicker/     # Gradient wallpaper picker component
    └── ...
```
...
| Drag & drop state context        | `packages/core/src/hooks/useDragDrop/DragDropStateContext.tsx`                    |
| File drop handler                | `packages/core/src/hooks/useGlobalDropHandler.ts`                                 |
| File operations (move/copy)      | `packages/core/src/hooks/useFileOperations.ts`                                    |
| Window drop zone hook            | `packages/core/src/hooks/useDragDrop/useWindowDropZone.ts`                        |
| Drop context menu                | `packages/core/src/components/Shared/DropFileMenu/DropFileMenu.tsx`               |
| Window drop overlay              | `packages/core/src/components/Window/WindowDropOverlay.tsx`                       |
| Unsaved changes modal            | `packages/core/src/components/Shared/UnsavedChangesModal/UnsavedChangesModal.tsx` |
| Wallpaper modules                | `packages/core/src/components/Apps/SettingsApp/sections/WPModule/`                |
| Gradient picker                  | `packages/core/src/components/Shared/GradientPicker/GradientPicker.tsx`           |
| Noise generation                 | `packages/core/src/utils/createNoise.ts`                                          |
