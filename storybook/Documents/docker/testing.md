...
```bash
# Test getRuntime
bun run test:run src/Shared/Utils/getRuntime.test.ts

# Test DockerFileSystemAdapter
bun run test:run src/Infrastructure/Adapters/DockerFileSystemAdapter.test.ts
...
| Unit tests          | `bun test:run`                           | All pass           |
