...
self.addEventListener('fetch', event => {
  ...
  // In development mode, always fetch fresh — never cache.
  // Cache-first strategy blocks Vite HMR updates because the SW
  // returns stale cached modules even after file changes.
  if (isDev()) {
    event.respondWith(fetch(event.request));
    return;
  }
