...
- **Components/**: UI components (Window, Taskbar, DesktopIcon, Apps, etc.)
  ...
  - **Shared/**: Reusable components (AppIcon, FilePickerApp, CreateItemApp, CollapsibleSection, etc.)
...
- **Store/**: Zustand stores (desktopStore.ts, settingsStore.ts, closeModalStore.ts)
...
### desktopStore (`src/Presentation/Store/desktopStore.ts`)
...
### settingsStore (`src/Presentation/Store/settingsStore.ts`)
...
### closeModalStore (`src/Presentation/Store/closeModalStore.ts`)
...
### AppTempMemory (`src/Presentation/Store/AppTempMemory.ts`)
...
Location: `src/Presentation/Store/factories/createWindowStateStore.ts`
...
- `notepad` (NotesApp with TipTap editor)
- `menuedit` (MenuEditApp with drag-drop)
- `calendar` (CalendarApp)
- `dos-emulator` / `doom` (DosEmulatorApp)
- `device-info` (DeviceInfoApp)
- `storybook` (StorybookApp)
- `linkedin` / `github` (ExternalLinkApp)

**Synchronously loaded apps:**

- `welcome` (WelcomeApp - default home)
- `files` (FilesApp - file manager)
...
```typescript
// AppRegistry.tsx uses React.lazy() for lazy apps
const NotesApp = lazy(() => import('@presentation/Components/Apps/NotesApp/NotesApp'));
```
...
| ------------ | ------------------------------ | -------------------------------- |
| API Server   | `src/Server/index.ts` (source) | `dist-server/index.js` (bundled) |
...
│  └── [vite] (Dev only) - HMR                                    │
│                                                                  │
│  Dev:  source code mounted, api from src/Server/                 │
│  Prod: bundled server (dist-server/), static files in image     │
└─────────────────────────────────────────────────────────────────┘
```
