...
- **Components/**: UI components (Window, Taskbar, DesktopIcon, Apps, etc.)
  ...
  - **Shared/**: Reusable components
    ...
    - **DownloadProgress/**: Download progress component (uses `useUnicodeAnimation`)
    - **UnicodeAnimation**: Thin wrapper over `useUnicodeAnimation` for rendering braille spinner frames
    - **DropFileMenu/**: Context menu for file drag & drop (move/copy/cancel with replace confirmation)
    - **ExternalLinkApp/**: Reusable external link app component
    - **FilePickerApp/**, **FolderPicker/**: File picker and folder picker modals
    - **GradientPicker/**: Gradient color picker with presets, custom angle, and noise overlay for wallpapers
    - **LanguageFlag/**: SVG flag components for language selection
    - **SettingsSection/**: Reusable settings section wrapper
    - **UnsavedChangesModal/**: Modal dialog for unsaved changes (save/discard/cancel)
...
- **Utils/**: Utility functions (getFileExtension, sortNodes, getAppIdForMime, buildGradientCSS, createNoiseDataUrl, resolveFileContent, buildBreadcrumbs, buildPath)
...
- **pendingFsDrop**: Pending file drop for cross-navigation coordination (nodeId, targetFolderId, position)
- **desktopFolderId**: Root folder ID for desktop icon file operations

### settingsStore (`packages/core/src/stores/settingsStore.ts`)

User preferences and theme settings:

- **wallpaper**: Background image URL or CSS gradient
...
- **wallpaperModule**: Active wallpaper module ('default' for static image | 'gradient' for gradient wallpaper)
- **gradientConfig**: Gradient wallpaper configuration (type, colors, angle, noise settings)
...
Complete system for dragging files between desktop, windows, and the file explorer:

- **useFileOperations**: Move/copy files with replace confirmation
- **useGlobalDropHandler**: Central dispatcher routing drops to windows or desktop menu
- **useWindowDropZone**: Makes windows drop targets with MIME compatibility checking
- **WindowDropOverlay**: Visual feedback showing compatible/incompatible drop targets
- **DropFileMenu**: Context menu for desktop drops (move/copy/cancel)
- **UnsavedChangesModal**: Confirmation dialog for file replace operations
- **Extended DragDropStateContext**: Position tracking via `PointerSensor`, `'file'` drag type, `'window'` drop target type

### Wallpaper Module System (2025)

Two-mode wallpaper system with gradient support and noise generation:

- **StaticWallpaperModule**: Preset images or custom upload
- **GradientWallpaperModule**: Custom gradients with presets, angles, and noise overlay
- **GradientPicker**: Full gradient editing component with presets
- **createNoiseDataUrl**: Device pixel ratio-aware noise texture generation
- **buildGradientCSS** / **isCSSGradient**: Gradient CSS computation utilities
- Smart module switching preserves last static wallpaper

### Multi-Architecture Docker (2025)

Docker images now support `linux/amd64` and `linux/arm64`:

- Automated multi-arch builds via Docker Buildx + QEMU in CI/CD
- Auto-tagging with `docker/metadata-action` (`vX.Y.Z`, `X.Y`, `latest`)
- Manual publish script: `./scripts/docker-publish.sh <tag>`

### macOS DMG Distribution (2025)

Tauri 2 builds produce universal `.dmg` installers for macOS:

- Universal binary covering `arm64` (Apple Silicon) and `x64` (Intel)
- Built on `macos-latest` runner in CI/CD
- First launch requires right-click > Open when unsigned

### CI/CD Improvements (2025)

- **npm-publish.yml**: Now syncs `src/` to the `fde-demo` repo after publishing, updating the package version automatically
- **release.yml**: npm publish step moved after create-tag to prevent native build failures from blocking npm publication
...
The desktop supports dragging files between the desktop, windows, and the file explorer. This is a comprehensive system built on `@dnd-kit/core` with a custom context and multiple specialized hooks.

#### Architecture

```
┌─────────────────────────────────────────────────────────────────────┐
│                     DragDropProvider (FdeDesktop)                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐ │
│  │ DesktopIcon   │  │ FileList     │  │ Window (droppable)       │ │
│  │ (draggable)  │  │ (draggable)  │  │  └─ WindowDropOverlay    │ │
│  │              │  │              │  │     compatible/incompat.  │ │
│  └──────┬───────┘  └──────┬───────┘  └────────────┬─────────────┘ │
│         │                  │                        │               │
│         ▼                  ▼                        ▼               │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │              DragDropStateContext                             │  │
│  │  activeItem · overTarget · pointerSensor · dropPosition      │  │
│  └──────────────────────────┬───────────────────────────────────┘  │
│                              ▼                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │              useGlobalDropHandler                              │  │
│  │  Routes drops to:                                             │  │
│  │  • Window → open file in app (if compatible)                  │  │
│  │  • Desktop → show DropFileMenu (move/copy)                   │  │
│  │  • Incompatible → show cancellation overlay                  │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
1. **Drag starts**: User drags a file icon from `DesktopIcon` or `FileList` → `buildDragItem()` creates `DragItemData` with type `'file'`, MIME info, and source metadata
2. **Drag over window**: `useWindowDropZone` checks MIME compatibility → renders `WindowDropOverlay` (green = compatible, red = incompatible)
3. **Drop on window**: `useGlobalDropHandler` routes to the correct app (e.g., ImageViewer for `.png`) via `getAppIdForMime`
4. **Drop on desktop**: Shows `DropFileMenu` with move/copy options via `useDropMenu`
5. **File operations**: `useFileOperations` handles move, copy, and replace confirmation with `UnsavedChangesModal`

#### Key Components

| Component              | Location                                 | Purpose                                                               |
| ---------------------- | ---------------------------------------- | --------------------------------------------------------------------- |
| `DragDropStateContext` | `hooks/useDragDrop/`                     | Context provider tracking active item, over target, and drop position |
| `WindowDropOverlay`    | `components/Window/`                     | Visual feedback overlay when dragging files over windows              |
| `DropFileMenu`         | `components/Shared/DropFileMenu/`        | Context menu offering move/copy/cancel with replace confirmation      |
| `UnsavedChangesModal`  | `components/Shared/UnsavedChangesModal/` | Confirmation dialog for unsaved file changes                          |

#### Key Hooks

| Hook                       | Purpose                                                                  |
| -------------------------- | ------------------------------------------------------------------------ |
| `useGlobalDropHandler`     | Central dispatcher for all file drop events across the desktop           |
| `useFileOperations`        | Move, copy, find existing nodes, and replace confirmation logic          |
| `useDropMenu`              | State management for the drop context menu (position, visibility)        |
| `useWindowDropZone`        | Makes a window a drop target with MIME compatibility checking            |
| `usePendingFsDropConsumer` | Consumes pending drops when navigating to the target folder in Files app |
| `buildDragItem`            | Factory function creating `DragItemData` objects with proper metadata    |
...
The `onDrop` callback now receives a position parameter `{ x, y }` captured via `PointerSensor` for precise drop placement on the desktop.

### Wallpaper Module System

The Settings app supports two wallpaper modes through a modular architecture:

| Module   | Component                 | Purpose                                                                   |
| -------- | ------------------------- | ------------------------------------------------------------------------- |
| Static   | `StaticWallpaperModule`   | Select from preset images or upload a custom wallpaper                    |
| Gradient | `GradientWallpaperModule` | Create gradient wallpapers with presets, custom angles, and noise overlay |

#### Gradient Engine

- **GradientPicker**: Component with preset gradients, custom angle control, and multi-stop color editing
- **buildGradientCSS**: Converts gradient configuration objects to CSS `linear-gradient()` / `radial-gradient()` strings
- **createNoiseDataUrl**: Generates a noise texture overlay as a data URL (device pixel ratio-aware) for adding visual texture to gradients

#### Noise Generation

The `createNoiseDataUrl` utility generates visual noise that can be overlaid on gradient wallpapers:

```typescript
import { createNoiseDataUrl } from '@fde-desktop/fde-core';

// Generate a 128x128 noise texture with 15% intensity
const noiseUrl = createNoiseDataUrl(128, 128, 0.15);
// Use as CSS background overlay
```

The noise is rendered at a reduced resolution and scaled up for performance, with automatic device pixel ratio handling.

#### Settings Store State

```typescript
const { wallpaperModule, gradientConfig } = useSettingsStore();
// wallpaperModule: 'default' (static image) | 'gradient'
// gradientConfig: { type, angle, colors, addNoise, noiseOpacity }
```

Smart module switching preserves the last static wallpaper when switching to gradient and restores it when switching back.
...
- **Settings**: Appearance, launcher, font, wallpaper modules (static/gradient with noise), and reset settings
- **Storybook**: Storybook playground
- **Terminal**: Interactive terminal emulator (xterm.js + WebSocket PTY). Docker only — shows an unsupported notice in web/Tauri
...
5. `npm-publish.yml` → syncs `src/` to `fde-demo` repo with updated `@fde-desktop/fde-core` version
...
Docker images are built for **linux/amd64** and **linux/arm64** using Docker Buildx with QEMU emulation:

- **CI/CD**: Automated via `docker-publish` job in `release.yml`
- **Tags**: Automatic `vX.Y.Z`, `X.Y`, `latest` via `docker/metadata-action`
- **Cache**: GHA cache enabled for faster rebuilds
- **Local**: Use `./scripts/docker-publish.sh v0.4.26` for manual multi-arch builds

Users on Mac ARM can run the native `arm64` image without Rosetta using Podman.

### macOS DMG Distribution

Tauri 2 builds produce a universal `.dmg` installer for macOS:

- **Universal binary**: covers `arm64` (Apple Silicon) and `x64` (Intel) in a single bundle
- **CI/CD**: Build runs on `macos-latest` runner
- **Unsigned**: users must right-click > Open on first launch when not signed/notarized
- **Artifact naming**: `${productName}-${version}-universal.dmg`
...
export function getRuntime(): 'browser' | 'docker' | 'tauri' {
...
│  ┌─────────────────┐  ┌─────────────────┐  ┌────────────────┐  │
│  │ IndexedDBFS     │  │ DockerFS        │  │ TauriFS        │  │
│  │ (Browser)       │  │ (Container)      │  │ (Desktop)      │  │
│  │                 │  │                  │  │                │  │
│  │ - IndexedDB API │  │ - fetch('/api') │  │ - Tauri fs/api │  │
│  │ - Blob storage  │  │ - REST endpoints │  │ - plugin-fs    │  │
│  └─────────────────┘  └─────────────────┘  └────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
