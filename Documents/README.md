...
| **Hooks**      | 35+ hooks for opening apps, notifications, icons, file system, drag & drop, etc.        |
...
- **Tauri**: Native desktop app (Tauri 2 filesystem APIs)

### State Management

Multiple Zustand stores with persistence:

- `desktopStore`: Windows, icons, clipboard, pending file drops
- `settingsStore`: User preferences, theme, fonts, wallpaper modules
...
Files can be dragged between the desktop, windows, and the file explorer. The system uses `@dnd-kit/core` with hooks for move/copy operations, MIME-based compatibility checking, and visual overlay feedback. See [architecture.md](./architecture.md) for the full D&D architecture.

### Wallpaper Modules

Two wallpaper modes are supported: static images and gradient wallpapers with noise overlays. See [architecture.md](./architecture.md) for the wallpaper module system details.
