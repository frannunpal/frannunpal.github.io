...
Creating an app has four steps:
...
```typescript
{
  ...
  defaultWidth: 800,        // Default window width (uses DEFAULT_WINDOW_DIMENSIONS)
  defaultHeight: 600,       // Default window height
  minWidth: 640,            // Minimum width
  minHeight: 480,           // Minimum height
  canMaximize: true,        // Whether it can be maximized (defaults to true)
  hasContextMenu: false,    // Whether desktop icon has context menu (for Files app)
}
```

Most apps use `DEFAULT_WINDOW_DIMENSIONS` which provides sensible defaults (800x600, min 640x480).
