...
| -------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `window.__FDE_IN_DOCKER__` | `boolean` | `true` when running in Docker, `false` in local browser/Tauri                                                                            |
...
| -------------- | --------- | ------------------------------------------------------------------ |
| `getRuntime()` | `Runtime` | Returns `'docker'`, `'browser'`, or `'tauri'`                      |
| `isDocker()`   | `boolean` | `true` when running inside Docker                                  |
| `isBrowser()`  | `boolean` | `true` in standard web browser (not Docker, not Tauri)            |
| `isTauri()`    | `boolean` | `true` when running inside a Tauri native app                     |
...
```typescript
import { getRuntime, isDocker, isBrowser, isTauri, isDev } from '@shared/Utils/getRuntime';

// getRuntime() — returns 'docker' | 'browser' | 'tauri'
...
// isBrowser() — true in standard web browser (not Docker, not Tauri)
if (isBrowser()) {
  // IndexedDB-only features
}

// isTauri() — true when running inside a Tauri native app
if (isTauri()) {
