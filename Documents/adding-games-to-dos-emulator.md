...
To add a desktop icon for quick access, edit `src/Shared/Constants/apps.ts`:
...
3. **Add desktop icon** (optional):

   ```typescript
   // src/Shared/Constants/apps.ts
   {
     id: 'simcity2000',
     name: 'SimCity 2000',
     icon: '🏙️',
     fcIcon: 'FcCity',
     autoLoad: { url: `${import.meta.env.BASE_URL}Games/simcity2000.jsdos`, name: 'SimCity 2000' },
   },
   ```

4. **Register in AppRegistry** (if adding desktop icon):

   ```typescript
   // src/Presentation/Components/Window/AppRegistry.tsx
   import DosEmulatorApp from '@presentation/Components/Apps/DosEmulatorApp/DosEmulatorApp';

   // Add to registry:
   'simcity2000': {
     component: DosEmulatorApp,
     // No buildMenuBar needed - DOS emulator uses inline controls
   },
   ```
